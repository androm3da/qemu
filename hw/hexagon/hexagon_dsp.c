                                ...
                                hexagon_machine_config *m_cfg)
{
    ...
    /* Create L2VIC device early for interface linking */
    ...
    if (!object_property_set_link(OBJECT(glob_regs_dev), "l2vic-interface",
    ...
    for (int i = 0; i < machine->smp.cpus; i++) {
        ...
        if (!object_property_set_link(OBJECT(cpus[i]), "l2vic-interface",
                                      OBJECT(l2vic_dev), errp)) {
