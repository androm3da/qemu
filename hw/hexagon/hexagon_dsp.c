...
/*
 * These cores have twelve hardware threads but only four and eight HVX
 * extension contexts respectively, so they exercise the SSR:XA context
 * selection that a core with a context per thread never reaches.
 * V81QA_1 also has the eight contexts that are the most SSR:XA can
