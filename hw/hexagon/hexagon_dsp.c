...
static void SA8775P_cdsp0_config_init(MachineState *machine)
{
    ...
    sysbus_mmio_map(SYS_BUS_DEVICE(ipcc), 0, 0x60000000);

    /* Connect CDSP IRQ (client 0) to L2VIC IRQ 128 */
    Object *l2vic_obj = object_resolve_path_type("", TYPE_L2VIC, NULL);
    if (l2vic_obj) {
        DeviceState *l2vic = DEVICE(l2vic_obj);
        sysbus_connect_irq(SYS_BUS_DEVICE(ipcc), 0,
                          qdev_get_gpio_in(l2vic, 128));
    }
}
