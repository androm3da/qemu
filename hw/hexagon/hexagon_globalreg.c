...
static uint32_t hexagon_read_vid_from_globalreg(HexagonGlobalRegState *s,
                                                 uint32_t vid_reg)
{
    /* For now, always use stored register value */
    return s->regs[vid_reg];
}
...
uint32_t hexagon_globalreg_read(HexagonGlobalRegState *s, uint32_t reg)
{
    ...
    } else if (reg == HEX_SREG_VID) {
        value = hexagon_read_vid_from_globalreg(s, reg);
    } else if (reg == HEX_SREG_VID1) {
        value = hexagon_read_vid_from_globalreg(s, reg);
    } else {
        value = s->regs[reg];
    }
                                    ...
                                    uint32_t value)
{
    ...
    if (reg_mask != IMMUTABLE) {
        ...
        uint32_t masked_value = apply_write_mask(value, old_value, reg_mask);
        s->regs[reg] = masked_value;
    }
    trace_hexagon_globalreg_write(get_sreg_name(reg), s->regs[reg]);
}
...
void hexagon_globalreg_set_pcycle_base(HexagonGlobalRegState *s, uint64_t value)
{
    g_assert(s);
...
static void hexagon_globalreg_realize(DeviceState *dev, Error **errp)
{
    ...
    if (s->l2vic_dev) {
        Object *l2vic_obj = OBJECT(s->l2vic_dev);
        if (!object_dynamic_cast(l2vic_obj, TYPE_L2VIC_INTERFACE)) {
            error_setg(errp, "l2vic device must implement L2VIC interface");
            return;
        }
    }
}
...
static const Property hexagon_globalreg_properties[] = {
    ...
    DEFINE_PROP_UINT32("qtimer-base-addr", HexagonGlobalRegState,
                       qtimer_base_addr, 0),
    DEFINE_PROP_LINK("l2vic", HexagonGlobalRegState, l2vic_dev,
