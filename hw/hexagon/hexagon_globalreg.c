...
uint32_t hexagon_globalreg_read(HexagonGlobalRegState *s, uint32_t reg)
{
    ...
    if ((reg == HEX_SREG_VID) || (reg == HEX_SREG_VID1)) {
        ...
        value = l2vic_read_vid(s->l2vic_interface, vid_group);
                             ...
                             uint32_t value)
{
    ...
    if ((reg == HEX_SREG_VID) || (reg == HEX_SREG_VID1)) {
        ...
        if (s->l2vic_interface) {
            uint32_t vid_group = (reg == HEX_SREG_VID) ? 0 : 1;
            l2vic_update_vid(s->l2vic_interface, vid_group, value);
        }
    }
                                    ...
                                    uint32_t value)
{
    ...
    if ((reg == HEX_SREG_VID) || (reg == HEX_SREG_VID1)) {
        ...
        if (s->l2vic_interface) {
            uint32_t vid_group = (reg == HEX_SREG_VID) ? 0 : 1;
            l2vic_update_vid(s->l2vic_interface, vid_group, final_value);
        }
    }
...
static const Property hexagon_globalreg_properties[] = {
    ...
    DEFINE_PROP_LINK("qtimer-interface", HexagonGlobalRegState,
                     ...
                     QTimerInterface *),
    DEFINE_PROP_LINK("l2vic-interface", HexagonGlobalRegState,
