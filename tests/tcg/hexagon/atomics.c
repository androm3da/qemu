...
/*
 * QEMU emulates load-locked/store-conditional with a value comparison, so
 * only the cases that do not depend on a real reservation are checked here.
 * See "Multi-threaded TCG" in docs/devel/multi-thread-tcg.rst.
 */
static void test_reservations(void)
{
    int32_t value = 1;

    /* A store-conditional with no preceding load-locked must fail. */
    check32(store_conditional32(&value, 2), 0);
    check32(value, 1);

    /* Returning from a syscall drops the reservation. */
    check32(load_locked32(&value), 1);
    getpid();
    check32(store_conditional32(&value, 3), 0);
    check32(value, 1);
}
