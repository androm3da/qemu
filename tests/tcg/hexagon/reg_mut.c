...
static inline void write_control_registers(void)
{
    ...
    /*
     * utimerlo/utimerhi are a read-only free-running counter, so the write
     * must be discarded.  The read-back is whatever the timer says, so only
     * check that the written value did not stick.
     */
