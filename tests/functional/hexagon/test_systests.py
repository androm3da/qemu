...
class SysTestsStandaloneTests(QemuSystemTest):
    ...
    def check(self, test_name: str, expected_exitcode: int = 0) -> None:
        """
        ...
        if self.vm.exitcode() != expected_exitcode:
            raise RuntimeError(
                f"Test {test_name} exited with code {self.vm.exitcode()}, "
                f"expected {expected_exitcode}"
            )
    ...
    def run_individual_test(self, test_name: str, machine: str = "sim",
                            expected_exitcode: int = 0) -> bool:
        """
        ...
        try:
            self.check(test_name, expected_exitcode)
            return True
    ...
    # Machine-specific test variants
    # These correspond to the machine-specific invocations from the removed TCG tests
