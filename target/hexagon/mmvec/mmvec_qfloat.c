...
unfloat legacy_parse_qf32(int32_t in)
{
    ...
    out.sig = fabs(ldexp((double)signif, -23));
    return out;
}
...
unfloat legacy_parse_qf16(int16_t in)
{
    ...
    signif = ((int32_t)in >> 4) | 1;
    out.sig = fabs(ldexp((double)signif, -10));
    return out;
}

static double legacy_signed_sig(unfloat u)
{
    return u.sign ? -u.sig : u.sig;
}

static int legacy_add_exp(unfloat a, unfloat b, int frac)
{
    int exp;

    if (a.exp > b.exp) {
        exp = a.exp + (a.sig == 0.0 ? -(frac + 1) : ilogb(a.sig));
        return MAX(exp, b.exp);
    }
    exp = b.exp + (b.sig == 0.0 ? -(frac + 1) : ilogb(b.sig));
    return MAX(exp, a.exp);
}

static uint32_t legacy_rnd_sat_qf(int size, int exp, double sig,
                                   double sig_low)
{
    int frac = size == 32 ? 23 : 10;
    int emax = size == 32 ? 128 : 16;
    int emin = size == 32 ? -127 : -15;
    int bias = size == 32 ? BIAS_QF32 : BIAS_QF16;
    uint32_t mask = size == 32 ? 0x7fffff : 0x3ff;
    double epsilon = ldexp(1.0, size == 32 ? -23 : -10);
    double scale = 1.0;
    double sig_f = 0.0;
    int sign = sig < 0.0;
    int exp_adj = 0;
    ...
    if (exp == emin - 1 || (prod_ovf && exp < emax)) {
        scale = 2.0;
        exp_adj = 1;
    }

    double sig_s = sig / scale;
    double r1 = sig_s * ldexp(1.0, frac);
    double r2 = floor(r1 / 4.0) * 4.0;
    double r3 = r1 - r2;

    if (r3 == 0.0 && sig_low < 0.0) {
        sig_f = sig_s + (3.0 - r3 - 4.0) * epsilon;
    } else if (exp == emax || exp == emin - 1 || prod_ovf) {
        sig_f = sig_s + ((r3 < 2.0 ||
                          (r3 == 2.0 && sig_low <= 0.0)) ?
                         1.0 - r3 : 3.0 - r3) * epsilon;
    } else if (r3 < 1.5 || (r3 == 1.5 && sig_low <= 0.0)) {
        sig_f = sig_s + (1.0 - r3) * epsilon;
    } else if (r3 < 2.5 || (r3 == 2.5 && sig_low <= 0.0)) {
        sig_f = (sig + (2.0 - r3) * epsilon) * 0.5;
        exp_adj = 1;
    } else {
        sig_f = sig_s + (3.0 - r3) * epsilon;
    }

    uint64_t bits;
    memcpy(&bits, &sig_f, sizeof(bits));
    int dexp = ((bits >> 52) & 0x7ff) - 1023;
    uint64_t significand = ((bits & UINT64_C(0xfffffffffffff)) |
                            UINT64_C(0x10000000000000)) << 11;
    significand = dexp >= 0 ? significand << dexp : significand >> -dexp;
    sig_out = (significand >> (size == 32 ? 41 : 54)) & mask;
    if (sign) {
        sig_out = ~sig_out;
    }
...
{
    int exp = legacy_add_exp(a, b, size == 32 ? 23 : 10);
    double asig = ldexp(legacy_signed_sig(a), a.exp - exp);
    double bsig = ldexp(legacy_signed_sig(b), b.exp - exp);
    double sig = asig + bsig;
    double sig_low = a.exp > b.exp ? (asig - sig) + bsig :
                                      (bsig - sig) + asig;

    return legacy_rnd_sat_qf(size, exp, sig, sig_low);
}

uint32_t legacy_qf_mpy(int size, unfloat a, unfloat b)
{
    return legacy_rnd_sat_qf(size, a.exp + b.exp,
                             legacy_signed_sig(a) * legacy_signed_sig(b), 0);
}

int32_t legacy_conv_sf_qf32(uint32_t a)
{
    return conv_unfloat_to_ieee(legacy_parse_qf32(a), SF,
                                RND_TO_NEAREST_EVEN, UNITS32, EPSILON32,
                                E_MAX_SF, sf_MANTBITS);
}

int16_t legacy_conv_hf_qf32(uint32_t a)
{
    return conv_unfloat_to_ieee(legacy_parse_qf32(a), HF,
                                RND_TO_NEAREST_EVEN, UNITS16, EPSILON16,
                                E_MAX_HF, hf_MANTBITS);
}

int16_t legacy_conv_hf_qf16(uint16_t a)
{
    return conv_unfloat_to_ieee(legacy_parse_qf16(a), HF,
                                RND_TO_NEAREST_EVEN, UNITS16, EPSILON16,
                                E_MAX_HF, hf_MANTBITS);
}
