...
void HELPER(vhist)(CPUHexagonState *env)
{
    ...
    for (int lane = 0; lane < 8; lane++) {
        for (int i = 0; i < MAX_VEC_SIZE_BYTES / 8; ++i) {
            ...
            env->VRegs[regno].uh[(MAX_VEC_SIZE_BYTES / 16) * lane + element]++;
        }
    }
}
...
uint32_t HELPER(upcycle_read)(CPUHexagonState *env, uint32_t sreg)
{
    ...
    return sreg == HEX_SREG_PCYCLELO ?
        hexagon_get_sys_pcycle_count_low(env) :
        hexagon_get_sys_pcycle_count_high(env);
}
